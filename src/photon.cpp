    ...
    if (object_exists(tgroup, "transitions")) {
      ...
      if (n_transition > 0) {
        ...
        for (int j = 0; j < n_transition; ++j) {
          ...
          if (j > 0)
            shell.cum_probability[j] = (matrix(j, 3) / norm) + shell.cum_probability[j - 1];
          else
            shell.cum_probability[j] = (matrix(j, 3) / norm);
        }
      }
    }
    close_group(tgroup);
...
void PhotonInteraction::atomic_relaxation(int i_shell, Particle& p) const
{
  ...
  while (n_holes > 0) {
    ...
    for (int i_trans = 0; i_trans < cum_probabilities.size(); ++i_trans) {
